...
namespace ImDui {
    ...
    struct D2DRender {
        ...
        bool Init(ID2D1Factory* pD2DFactory, IDWriteFactory* pDWriteFactory, IWICImagingFactory* pWICFactory, ID2D1RenderTarget* pMainRT) {
        ...
        void Resize(float w, float h) {
            ...
            if (SUCCEEDED(_pMainRT->QueryInterface(&hwndRT))) {
                hwndRT->Resize(D2D1::SizeU(w, h));
                hwndRT->Release( );
            }
        }
    ...
    void InitResources(ID2D1Factory* pD2DFactory, IDWriteFactory* pDWriteFactory, IWICImagingFactory* pWICFactory, ID2D1RenderTarget* pMainRT) {
    ...
    bool WindowCloseButton(bool* open) {
        ...
        const ImFloat4 col = style.Colors[(held && hovered) ? Color_ButtonActive : hovered ? Color_ButtonHovered
                                                                                           : Color_Button];
    ...
    bool BeginWindow(const char* name, bool* p_open, ImFloat2 pos, ImFloat2 size, float fill_alpha, ImDuiWindowFlags flags) {
        ...
        if (!window->Collapse && !(window->Flags & ImDuiWindowFlags_NoResize)) {
            ...
            resize_col = s_state.Styles.Colors[held ? Color_ResizeGripActive : hovered ? Color_ResizeGripHovered
                                                                                       : Color_ResizeGrip];
            if (held) {
    ...
    void SameLine(int column_x, int spacing_w) {
        ...
        if (column_x != 0) {
            ...
            if (spacing_w < 0)
                spacing_w = (int)s_state.Styles.ItemSpacing.x;
    ...
    bool Button(const char* label, ImFloat2 size) {
        ...
        DrawWidgetFrame(boundRect, (hovered && held) ? Color_ButtonActive : hovered ? Color_ButtonHovered
                                                                                    : Color_Button);
    ...
    void DrawSpriteFromSheet(ID2D1RenderTarget* pRT, ID2D1Bitmap* pBitmap, int characterIndex, int direction, int pattern, float x, float y, float scale = 1.0f) {
        if (!pBitmap)
            return;

        // Размеры всего спрайта
        D2D1_SIZE_F sheetSize = pBitmap->GetSize( );
        int columns = 3; // 3 кадра анимации (pattern)
        int rows = 4;    // 4 направления (direction)

        // **Внимание**: У RPG Maker VX Ace один PNG может содержать **до 8 персонажей в одном файле**
        int charactersPerRow = 4;    // Обычно в одном файле 4 персонажа в ряд
        int charactersPerColumn = 2; // В высоту два ряда персонажей

        // Размер **одного персонажа**
        float charWidth = sheetSize.width / charactersPerRow;
        float charHeight = sheetSize.height / charactersPerColumn;

        // Размер **одного кадра** в персонаже
        float frameWidth = charWidth / columns;
        float frameHeight = charHeight / rows;

        // **Вычисляем координаты обрезки**
        int characterX = characterIndex % charactersPerRow;
        int characterY = characterIndex / charactersPerRow;

        float srcX = characterX * charWidth + frameWidth * pattern;
        float srcY = characterY * charHeight + frameHeight * ((direction / 2) - 1);

        // Обрезаем нужную область
        D2D1_RECT_F srcRect = D2D1::RectF(srcX, srcY, srcX + frameWidth, srcY + frameHeight);
        D2D1_RECT_F destRect = D2D1::RectF(x, y, x + frameWidth * scale, y + frameHeight * scale);

        // Отрисовка
        pRT->DrawBitmap(pBitmap, destRect, 1.0f, D2D1_BITMAP_INTERPOLATION_MODE_LINEAR, srcRect);
    }

    void DrawImage1(ID2D1RenderTarget* pRT, ID2D1Bitmap* pBitmap, float x, float y, float w, float h) {
        if (!pBitmap || !pRT)
            return;
        ...
        pRT->DrawBitmap(pBitmap, D2D1::RectF(x, y, x + w, y + h));
    }

    void ImDui::CheckBoxWithImage(const char* label, bool* v, ID2D1Bitmap* icon, int characterIndex, int direction, int pattern,
                                  float checkBoxOffset, float iconScale, float spacing, float hoverScale) {
        Window* window = s_state.RenderWindow;
        if (window->Collapse)
            return;
        ...
        // **Сдвигаем чекбокс вправо**
        ImFloat4 check_bb(window->Layout.CursorPos.x + checkBoxOffset, window->Layout.CursorPos.y,
                          text_size.y + style.FramePadding.y * 2, text_size.y + style.FramePadding.y * 2);

        ItemSize(check_bb);
        SameLine(0, style.ItemInnerSpacing.x);

        ImFloat4 text_bb(window->Layout.CursorPos.x + checkBoxOffset, window->Layout.CursorPos.y + style.FramePadding.y,
                         text_size.x, style.FramePadding.y + text_size.y);
        ...
        if (*v) {
            ImFloat4 fillRect(check_bb.x + 4, check_bb.y + 4, check_bb.z - 8, check_bb.w - 8);
            s_state.Render->DrawRect(window->CRT, style.Colors[Color_WidgetActive], fillRect, true);
        }
        ...
        // **Рисуем PNG рядом с чекбоксом**
        if (icon) {
            float iconX = 20.0f; // Фиксированный отступ для PNG
            float iconY = check_bb.y;
            float scale = check_bb.w / 32.0f * iconScale;

            // Проверяем, наведен ли курсор
            bool isIconHovered = PtInRect(s_state.Events.MousePos, ImFloat4(iconX + window->Rect.x, iconY + window->Rect.y, check_bb.z, check_bb.w));

            // Используем `hoverScale`, если навели на иконку или чекбокс
            float finalScale = (hovered || isIconHovered) ? hoverScale : iconScale;

            DrawSpriteFromSheet(window->CRT, icon, characterIndex, direction, pattern, iconX, iconY, scale * finalScale);

            // **При наведении рисуем увеличенное изображение справа от чекбокса**
            if (hovered || isIconHovered) {
                float enlargedX = check_bb.x + 350.0f; // Отступ 350 пикселей
                float enlargedY = check_bb.y;
                float enlargedScale = scale * hoverScale * 2.0f; // Увеличенный масштаб

                DrawSpriteFromSheet(window->CRT, icon, characterIndex, direction, pattern, enlargedX, enlargedY, enlargedScale);
            }
        }

        // **Дополнительный отступ между чекбоксами**
        window->Layout.CursorPos.y += spacing;
    }
    ...
    void ImDui::CheckBoxWithImage(const char* label, bool* v, ID2D1Bitmap* icon, int characterIndex, int direction, int pattern) {
        ...
        // **Сдвигаем чекбокс ВПРАВО (например, на 50 пикселей)**
        float checkBoxOffset = 150.0f; // Измените на нужное значение
        ImFloat4 check_bb(window->Layout.CursorPos.x + checkBoxOffset, window->Layout.CursorPos.y,
                          text_size.y + style.FramePadding.y * 2, text_size.y + style.FramePadding.y * 2);
        ...
        // **PNG остается на месте, отступ 50 пикселей**
        if (icon) {
            float iconX = 20.0f; // PNG не двигаем, оставляем на 50 пикселях
            float iconY = check_bb.y;
            float scale = check_bb.w / 32.0f; // Подгоняем размер к высоте чекбокса

            // Определяем область изображения
            ImFloat4 iconRect(iconX, iconY, check_bb.z, check_bb.w);

            // Проверяем, наведен ли курсор на PNG
            bool isIconHovered = PtInRect(s_state.Events.MousePos, ImFloat4(iconX + window->Rect.x, iconY + window->Rect.y, check_bb.z, check_bb.w));

            // **Увеличиваем PNG при наведении**
            float hoverScale = isIconHovered ? 10.0f : 1.0f;
            hoverScale = 3.0f;
            DrawSpriteFromSheet(window->CRT, icon, characterIndex, direction, pattern, iconX, iconY, scale * hoverScale);

            // DrawSpriteFromSheet(window->CRT, icon, characterIndex, direction, pattern, iconX, iconY, scale);
        }

        // **Дополнительный отступ между чекбоксами**
        window->Layout.CursorPos.y += 25; // 12 Увеличиваем расстояние между элементами
    }
    ...
    bool Collapse(const char* label, const char* str_id, const bool display_frame, const bool default_open) {
        ...
        // Render
        const ImFloat4 col = style.Colors[(held && hovered) ? Color_CollapseActive : hovered ? Color_CollapseHovered
                                                                                             : Color_Collapse];
        if (display_frame) {
            DrawWidgetFrame(bb, (held && hovered) ? Color_CollapseActive : hovered ? Color_CollapseHovered
                                                                                   : Color_Collapse);
    ...
    const char* GetStyleColorName(ImUint idx) {
        ...
        switch (idx) {
            ...
            case Color_Border:
                return "Color_Border";
            case Color_WindowBg:
                return "Color_WindowBg";
            case Color_WidgetBg:
                return "Color_WidgetBg";
            case Color_WidgetActive:
                return "Color_WidgetActive";
            case Color_TitleBar:
                return "Color_TitleBar";
            case Color_TitleBarCollapsed:
                return "Color_TitleBarCollapsed";
            case Color_Slider:
                return "Color_Slider";
            case Color_SliderActive:
                return "Color_SliderActive";
            case Color_Button:
                return "Color_Button";
            case Color_ButtonHovered:
                return "Color_ButtonHovered";
            case Color_ButtonActive:
                return "Color_ButtonActive";
            case Color_Collapse:
                return "Color_Collapse";
            case Color_CollapseHovered:
                return "Color_CollapseHovered";
            case Color_CollapseActive:
                return "Color_CollapseActive";
            case Color_ResizeGrip:
                return "Color_ResizeGrip";
            case Color_ResizeGripHovered:
                return "Color_ResizeGripHovered";
            case Color_ResizeGripActive:
                return "Color_ResizeGripActive";
            case Color_TooltipBg:
                return "Color_TooltipBg";
        }
        assert(0);
        return "Unknown";
    }
    ...
    void ShowStyleEditor( ) {
        ...
        for (size_t i = 0; i < Color_COUNT; i++) {
            ...
            if (memcmp(&style.Colors[i], &def.Colors[i], sizeof(ImFloat4)) != 0) {
                ImDui::SameLine( );
                if (ImDui::Button("Revert"))
                    style.Colors[i] = def.Colors[i];
            }
        }
    }
    ...
    Window::Window(const char* name, ImFloat2 default_pos, ImFloat2 default_size)
        : CRT(NULL), Rect(default_pos.x, default_pos.y, default_size.x, default_size.y), Alpha(1.f), Visible(true), Collapse(false), ItemWidthDefault(0.f) {
        Name = strdup(name);
        Resize(default_size);
    }
